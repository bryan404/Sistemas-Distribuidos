...
def load_parameters(config_path):
    config = np.genfromtxt(config_path, delimiter=',', skip_header=0)
    sigma = config[4]  # Line 5 (index 4 in zero-based indexing)
    top_k = int(config[5])  # Line 6 (index 5 in zero-based indexing)
    return sigma, top_k
...
def main():
    ...
    sigma, top_k = load_parameters(config_path)
